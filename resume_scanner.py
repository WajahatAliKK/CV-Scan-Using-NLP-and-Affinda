...
class ResumeScanner():
    ...
    def best_practices(self):
        ...
        # #### Phone Number
        matchNo = re.findall(
            '([+]\d{2}[-\.\s]??\d{3}[ ]??[-\.\s]??[ ]??\d{3}[-\.\s]??[ ]??\d{4}|[+]\d{1}[-\.\s]??\d{3}[-\.\s]??\d{3}['
            '-\.\s]??\d{4}|[-\.\s]??[(]??\d{3}[)]??[-\.\s]??\d{3}[-\.\s]??\d{4}|[+]\d{2}[-\.\s]??\d{3}[-\.\s]??\d{3}['
            '-\.\s]??\d{4}|\d{3}[-\.\s]??\d{3}[-\.\s]??\d{4}|\(\d{3}\)\s*\d{3}[-\.\s]??\d{4}|\d{3}[-\.\s]??\d{4}|[+] '
        ...
        matchEd = []
        Ed2 = re.search(
            '((bachelor|bachelors| bs|B.S.|B.A.|B.SC|B.Sc|BS|MSc|M.Sc|MS|PhD|doctor of '
            'philosophy|Major|Minor|University |(.*?) Centre|(.*?) Law School|(.*?) '
            'School|Academy|Department|Bachelor|BACHELOR|Master|MASTER|(.*?) College|University of)[\w -]*)',
            line)
        if Ed2 != None:
            Ed2 = Ed2.group(0)
        matchEd.append(Ed2)
        ...
        # #### Date Formatting
        matchEDD = re.findall(
            r'((1[0-2])\/(\d{4}))|((0[1-9])\/(\d{4}))|((1[0-2])\/(\d{2}))|((0[1-9])\/(\d{2}))|(([1-9])\/(\d{2}))|((['
            r'1-9])\/(\d{4}))|((JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC) ([0-9]{4}))|(('
            r'January|February|March|April|May|June|July|August|September|October|November|December) ([0-9]{4}))|(('
            r'JANUARY|FEBRUARY|MARCH|APRIL|MAY|JUNE|JULY|AUGUST|SEPTEMBER|OCTOBER|NOVEMBER|DECEMBER) ([0-9]{4}))',
            line)  # M/YYYY
        if matchEDD == [] or matchEDD == " ":
            self.resume_scores['best_practices'][
                'date_format'] = "We could not find a date that matched a recommended format. We recommend using one " \
                                 "of the following formats: 'M/YY' or 'MM/YY' or 'M/YYYY' or 'MM/YYYY' or 'Month " \
                                 "YYYY'. "
        ...
        if (orgName in jobDescTemp and orgURL in jobDescTemp):
            self.resume_scores['best_practices'][
                'organisation'] = f"The organization Name '{orgName}' and URL {orgURL} are present in job description"
            self.ATSp += 2.5
        elif orgName in jobDescTemp:
            self.resume_scores['best_practices'][
                'organisation'] = f"The organization Name '{orgName}' is present in job description"
            self.ATSp += 2.5
        elif orgURL in jobDescTemp:
            self.resume_scores['best_practices'][
                'organisation'] = f"The organization URL '{orgURL}' is present in job description"
            self.ATSp += 2.5
        else:
            self.resume_scores['best_practices'][
                'organisation'] = f"Organization Name and URL are not present in job description"
        ...
        if len(unkR) > 0:
            self.resume_scores['best_practices'][
                'organisation'] = f"Unknown/Unreadable Characters are present in your CV/Resume: " + ''.join(
                u for u in unkR)
        else:
            self.ATSp += 2.5
            self.resume_scores['best_practices'][
                'organisation'] = f"No Unknown/Unreadable Characters are present in your CV/Resume"
