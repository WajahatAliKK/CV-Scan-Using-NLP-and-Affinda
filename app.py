...
def scan_resume():
    try:
        ...
        if data.get('user_id', None) is None:
            return {"success": False, "msg": "Required file(s) missing, try again."}, 500
        ...
        if cv.filename.split('.')[-1] not in app.config['ALLOWED_FILES_TYPES']:
            return {"success": False, "msg": "Resume file type not allowed, try again."}, 500
        cv_binary = io.BytesIO(cv.read())  # for the s3 upload thread

        jd = request.files['jd']
        if jd.filename.split('.')[-1] not in app.config['ALLOWED_FILES_TYPES']:
            return {"success": False, "msg": "Job desc file type not allowed, try again."}, 500
        jd_binary = io.BytesIO(jd.read())  # for the s3 upload thread

        print('starting scan')
        resume_scanner_results = ResumeScanner(
            resume_file=cv, jd_file=jd, domain=data['job_title'], industry=data['industry_title'],
            affinda_token=app.config['AFFINDA_TOKEN'], temp_folder=app.config['TEMP_FOLDER']).start_scan()

        if not resume_scanner_results["success"]:
            return jsonify({"success": False, "msg": resume_scanner_results['msg']}), 401
